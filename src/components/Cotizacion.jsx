...
const FormularioProyecto = () => {
  ...
  useEffect(() => {
    axios
      .get(
        "https://planosperu.com.pe/intranet/api/ubicacion/?dep_ubi__isnull=false&pro_ubi__isnull=true&dis_ubi__isnull=true"
      )
      .then((response) => {
        const deps = response.data.results || response.data;
        const uniqueDeps = [
          ...new Map(deps.map((dep) => [dep.dep_ubi, dep])).values(),
        ];
        const options = uniqueDeps.map((dep) => ({
          value: dep.dep_ubi,
          label: dep.dep_ubi,
        }));
  ...
  const cargarProvincias = (departamento) => {
    ...
    axios
      .get(
        `https://planosperu.com.pe/intranet/api/ubicacion/?dep_ubi=${departamento}&pro_ubi__isnull=false&dis_ubi__isnull=true`
      )
      .then((response) => {
        const pros = response.data.results || response.data;
        const uniquePros = [
          ...new Map(pros.map((prov) => [prov.pro_ubi, prov])).values(),
        ];
        const options = uniquePros.map((prov) => ({
          value: prov.pro_ubi,
          label: prov.pro_ubi,
        }));
  ...
  const cargarDistritos = (departamento, provincia) => {
    ...
    axios
      .get(
        `https://planosperu.com.pe/intranet/api/ubicacion/?dep_ubi=${departamento}&pro_ubi=${provincia}&dis_ubi__isnull=false`
      )
      .then((response) => {
  ...
  const tipoProyectoOptions = [
    ...
    {
      value: "Declaratoria de fábrica por subdivisión",
      label: "Declaratoria de fábrica por subdivisión",
    },
    {
      value: "Declaratoria de fábrica por independización",
      label: "Declaratoria de fábrica por independización",
    },
    {
      value: "Levantamiento de cargas técnicas(no legales)",
      label: "Levantamiento de cargas técnicas(no legales)",
    },
    { value: "Búsqueda catrastal", label: "Búsqueda catrastal" },
    {
      value: "Acumulación de lote urbano",
      label: "Acumulación de lote urbano",
    },
      ...
      if (valor.length === 8) {
        try {
          const res = await fetch(
            `https://planosperu.com.pe/intranet/api/dni/${valor}`
          );
          const data = await res.json();
          if (data?.nombreCompleto) {
            setFormData((prev) => ({
              ...prev,
              cliente: capitalizarNombre(data.nombreCompleto),
            }));
          } else {
        ...
        try {
          const res = await fetch(
            `https://planosperu.com.pe/intranet/api/ruc/${valor}`
          );
          const data = await res.json();
          if (data?.razonSocial) {
            setFormData((prev) => ({
              ...prev,
              cliente: capitalizarNombre(data.razonSocial),
            }));
          } else {
            setFormData((prev) => ({ ...prev, cliente: "" }));
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://planosperu.com.pe/intranet/api/enviar/",
        {
          method: "POST",
          headers: {
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <Select
                  ...
                  value={
                    tipoProyectoOptions.find(
                      (option) => option.value === formData.tipoProyecto
                    ) || null
                  }
              ...
              <div className="form-group">
                ...
                <Select
                  ...
                  value={
                    departamentos.find(
                      (dep) => dep.value === formData.departamento
                    ) || null
                  }
              ...
              <div className="form-group">
                ...
                <Select
                  ...
                  value={
                    provincias.find(
                      (prov) => prov.value === formData.provincia
                    ) || null
                  }
              ...
              <div className="form-group">
                ...
                <Select
                  ...
                  value={
                    distritos.find(
                      (dist) => dist.value === formData.distrito
                    ) || null
                  }
