    ...
    <section className="mapa-proyectos">
      <div className="container">
        ...
        <div className="mapa-header text-center">
          ...
          <p className="mapa-subtitulo">Cada punto en este mapa representa un proyecto exitoso que hemos entregado, y seguimos expandiendo nuestra huella</p>
        </div>
        ...
        <div className="mapa-controles">
          ...
          <select value={ciudadSeleccionada} onChange={(e) => setCiudadSeleccionada(e.target.value)} className="mapa-select">
        ...
        <div className="mapa-container">
          <div className="mapa-imagen">
            <MapContainer center={[-9.19, -75.0152]} zoom={5} className="mapa-img" scrollWheelZoom={false} doubleClickZoom={false} dragging={true} touchZoom={false} zoomControl={false}>
              <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution="© OpenStreetMap" />

              <ChangeView center={ciudadSeleccionada ? getCentroDepartamento(ciudadSeleccionada) : [-9.19, -75.0152]} zoom={ciudadSeleccionada ? 10 : 5} />

              {proyectos
                .filter((p) => !ciudadSeleccionada || p.properties.departamento === ciudadSeleccionada)
                .map((p, i) => (
                  <Marker key={i} icon={puntoIcono} position={[p.geometry.coordinates[1], p.geometry.coordinates[0]]} />
                ))}
            </MapContainer>
          </div>
        </div>
